...
draws_df_summary <-
  draws_df |>
  summarize_draws(
    mean, median, sd, ~ quantile2(.x, probs = c(0.025, 0.975)),
...
saveRDS(
  ermod_bin_cov_sel_kfold,
  file = here("output", "ermod_bin_cov_sel_kfold.rds")
)
```
...
ermod_bin_cov_sel_kfold <- readRDS(
  here("output", "ermod_bin_cov_sel_kfold.rds")
)
```
