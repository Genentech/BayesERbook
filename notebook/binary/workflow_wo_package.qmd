...
set.seed(1234)
mat_epred <- rstanarm::posterior_epred(ermod_bin, newdata = data_for_sim)
...
sim_epred_med_qi <-
  ...
  dplyr::left_join(
    tibble::tibble(
      .row   = rep(seq_len(nrow(data_for_sim)), each = nrow(mat_epred)),
      .draw  = rep(seq_len(nrow(mat_epred)), times = nrow(data_for_sim)),
      .epred = as.vector(mat_epred)
    ),
    by = ".row"
  ) |>
  dplyr::group_by(dplyr::across(!c(.row, .draw, .epred))) |>
  ggdist::median_qi(.epred, .width = 0.95) |>
  dplyr::as_tibble()
```
...
# At least exposure metric should be included
...
set.seed(1234)
mat_epred_cov <- rstanarm::posterior_epred(
  ermod_bin_cov, newdata = data_for_sim
)
...
sim_epred_raw <-
  ...
  dplyr::left_join(
    tibble::tibble(
      .row   = rep(seq_len(nrow(data_for_sim)), each = nrow(mat_epred_cov)),
      .draw  = rep(seq_len(nrow(mat_epred_cov)), times = nrow(data_for_sim)),
      .epred = as.vector(mat_epred_cov)
    ),
    by = ".row"
  )
