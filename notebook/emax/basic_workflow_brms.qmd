...
Clear from inspection that there is no need for a sigmoidal model here: the posterior mean for `gamma` is `r round(summary(sigmoidal_model_fit)$fixed[4, 1], 2)` with 95% credible interval from `r round(summary(sigmoidal_model_fit)$fixed[4, 3], 2)` to `r round(summary(sigmoidal_model_fit)$fixed[4, 4], 2)`. However, if an explicit model comparison is required, cross-validation methods such as LOO-CV can be used to compare `sigmoidal_model_fit` to `continuous_base_fit`. This is discussed in the next chapter.
