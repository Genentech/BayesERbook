...
newdata_cnt_a <- expand_grid(
  exposure = seq(0, 50000, 1000),
  cnt_a = c(2, 5, 8),
  cnt_b = 5,
  cnt_c = 5
)
mat_cnt_a <- posterior_epred(continuous_covariate_fit, newdata = newdata_cnt_a)
...
sim_exposure_cnt_a <-
  ...
  left_join(
    tibble(
      .row  = rep(seq_len(nrow(newdata_cnt_a)), each = nrow(mat_cnt_a)),
      .draw = rep(seq_len(nrow(mat_cnt_a)), times = nrow(newdata_cnt_a)),
      .epred = as.vector(mat_cnt_a)
    ),
    by = ".row"
  ) |>
  group_by(exposure, cnt_a, cnt_b, cnt_c) |>
...
newdata_cnt_a_bin <- expand_grid(
  exposure = seq(0, 50000, 1000),
  cnt_a = c(2, 5, 8),
  cnt_b = 5,
  cnt_c = 5
)
mat_cnt_a_bin <- posterior_epred(
  binary_covariate_fit, newdata = newdata_cnt_a_bin
)
...
sim_exposure_cnt_a <-
  ...
  left_join(
    tibble(
      .row = rep(
        seq_len(nrow(newdata_cnt_a_bin)), each = nrow(mat_cnt_a_bin)
      ),
      .draw = rep(
        seq_len(nrow(mat_cnt_a_bin)), times = nrow(newdata_cnt_a_bin)
      ),
      .epred = as.vector(mat_cnt_a_bin)
    ),
    by = ".row"
  ) |>
  group_by(exposure, cnt_a, cnt_b, cnt_c) |>
